...
RPG.Beings.BaseBeing.prototype.serialize = function(serializer) {
	var result = {};
	if (this._name) { result.name = this._name; }
	if (this._cell) { result.cell = serializer.serialize(this._cell); }
	result.gender = this._gender;
	result.confirm = this._confirm;
	result.alive = this._alive;
	result.stats = this._stats;
	result.race = serializer.serializeClass(this._race.constructor);

	result.items = serializer.serializeArray(this._items);
	result.traps = serializer.serializeArray(this._knownTraps);
	result.modifierList = serializer.serializeArray(this._modifierList);
	result.effects = serializer.serializeArray(this._effects);
	
	result.spells = [];
	for (var i=0;i<this._spells.length;i++) { 
		result.spells.push(serializer.serializeClass(this._spells[i]));
	}
	
	result.feats = {};
	for (var p in this._feats) {
		result.feats[p] = serializer.serialize(this._feats[p]);
	}

	result.slots = {};
	var slots = this.getSlots();
	for (var p in slots) {
		var item = slots[p].getItem();
		if (item) { result.slots[p] = serializer.serialize(item); }
	}

	return result;
}

RPG.Beings.BaseBeing.prototype.revive = function(data, parser) {
	var race = parser.parseClass(data.race);
	return new this.constructor(new race());
}

RPG.Beings.BaseBeing.prototype.parse = function(data, parser) {
	if (data.name) { this._name = data.name; }
	if (data.cell) { this._cell = parser.parse(data.cell); }
	...
	for (var i=0;i<data.spells.length;i++) {
		var spell = parser.parseClass(data.spells[i]);
		this._spells.push(spell);
	}
	
	var slots = this.getSlots();
	for (var p in data.slots) {
		var item = parser.parse(data.slots[p]);
		slots[p].setItem(item);
	}
	
	this._items = [];
	for (var i=0;i<data.items.length;i++) {
		var item = parser.parse(data.items[i]);
		this._items.push(item);
	}

	for (var i=0;i<data.traps.length;i++) {
		var trap = parser.parse(data.traps[i]);
		this._knownTraps.push(trap);
	}
	
	for (var p in data.feats) {
		this._feats[p] = parser.parse(data.feats[p]);
	}
	
	this._modifierList = [];
	for (var i=0;i<data.modifierList.length;i++) {
		var modifier = parser.parse(data.modifierList[i]);
		this._modifierList.push(modifier);
	}
	
	this._effects = [];
	for (var i=0;i<data.effects.length;i++) {
		var effect = parser.parse(data.effects[i]);
